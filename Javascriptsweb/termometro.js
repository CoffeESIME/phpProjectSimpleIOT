	...
	function _initAxis() {
		...
		var domain = [
			1*step * Math.floor(this._value.min / (1*step)),
			1*step * Math.ceil(this._value.max / (1*step))
		];
		...
		var ticksAt = d3.range((domain[1] - domain[0]) / step + 1).map(function (v) {
			return 1.0*(domain[0] + v* 3*step);
//            aqui podemos editar los elementos del tamaño de medida del termometro multiplicando un numero por v y step (v*numero*step)
		});
	...
	function _renderAxis() {
		...
		var axis = d3.axisLeft(this._axisData.scale)
			.tickSizeInner(4)
//        aqui editamos para el espacio de los numeros y que se pueden ver todos 
			.tickSizeOuter(0)
